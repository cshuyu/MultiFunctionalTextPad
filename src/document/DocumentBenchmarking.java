...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps*increment + start; 
			...
			String text = getStringFromFile("data/warAndPeace.txt", numToCheck);
			long basicStartTime = System.nanoTime();	
			Document testBasicDoc = new BasicDocument(text);
			for(int numToTrial = 1; numToTrial <= trials; numToTrial++) {
				testBasicDoc.getFleschScore();
			}
			long basicEndTime = System.nanoTime();
			long basicTime = (basicEndTime - basicStartTime)/trials;
			System.out.print(basicTime + "\t");
			
			//System.gc();
			long efficientStartTime = System.nanoTime();
			Document testEfficientDoc = new EfficientDocument(text);
			for (int numToTrial = 1; numToTrial <= trials; numToTrial++) {
				testEfficientDoc.getFleschScore();
			}
			//testEfficientDoc = null;
			long efficientEndTime = System.nanoTime();
			long efficientTime = (efficientEndTime - efficientStartTime)/trials;
