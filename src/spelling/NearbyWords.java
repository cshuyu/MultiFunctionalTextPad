...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
			String newS = s + String.valueOf((char)charCode);
			if(!currentList.contains(newS) && 
					(!wordsOnly||dict.isWord(newS)) &&
					!s.equals(newS)) {
		...
		for(int index = 0; index < s.length(); index++){
			...
			for(int offset=0; offset<26; offset++) {
				StringBuffer sb = new StringBuffer(s);
				sb.insert(index, str, offset, offset+1);
				if(!currentList.contains(sb.toString()) && 
				(!wordsOnly||dict.isWord(sb.toString())) &&
				!s.equals(sb.toString())) {
					currentList.add(sb.toString());
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && numSuggestions>0 && limit>0) {
			word = queue.remove(0);
			List<String> suggestions = distanceOne(word,false);
